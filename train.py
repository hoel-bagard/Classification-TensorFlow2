...
def train(model, train_dataset, val_dataset, output_dir, steps_per_epoch, max_epoch=50):
    ...
    for epoch in range(max_epoch):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            one_hot_y_train = tf.one_hot(y_batch_train, output_classes)
            with tf.GradientTape() as tape:
                logits = model(x_batch_train, training=True)
                loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(one_hot_y_train, logits))
            grads = tape.gradient(loss, model.trainable_weights)
            optimizer.apply_gradients(zip(grads, model.trainable_weights))

            epoch_loss += loss
            epoch_progress = int(30 * (step/steps_per_epoch))
            print(f"{step}/{steps_per_epoch} [" + epoch_progress*"=" + ">" +
                  (30-epoch_progress)*"." + f"] - loss: {loss}" + 15*" ", end="\r", flush=True)

        # Also give how long it took
        print(f"\nEpoch loss: {epoch_loss/step}")

        acc_train = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(logits, axis=-1),
                                                    tf.cast(y_batch_train, dtype=tf.int64)), tf.float32))
        with file_writer.as_default():
            tf.summary.scalar("Loss/Train",          loss,          step=epoch)
            tf.summary.scalar("Accuracy/Train",      acc_train,     step=epoch)
            tf.summary.image("Train", x_batch_train, max_outputs=4, step=epoch, description=str(y_batch_train))
            file_writer.flush()

        # Validation every 10 epochs
        if epoch % 1 == 0 and epoch > 1:
            x_batch_val, y_batch_val = [sample for sample in val_dataset.take(1)][0]
            logits = model(x_batch_val, training=True)
            one_hot_y_val = tf.one_hot(y_batch_train, output_classes)
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(one_hot_y_val, logits))
            with file_writer.as_default():
...
def main():
    ...
    train_dataset = train_dataset.shuffle(buffer_size=1024).batch(args.batch_size, drop_remainder=True)

    # Prepare the validation dataset.
    val_dataset = tf.data.Dataset.from_tensor_slices((x_val, y_val))
    val_dataset = val_dataset.shuffle(buffer_size=1024).batch(args.batch_size, drop_remainder=True)
